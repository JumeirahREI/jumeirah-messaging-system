...
export const Route = createFileRoute(
  ...
  loader: async ({ params }) => {
    ...
    if (Number.isNaN(apartmentId)) throw new Error("معرّف شقة غير صالح")
    const [apartment, contacts, phoneNumbers, allContacts] = await Promise.all([
      getApartment({ data: { id: apartmentId } }),
      listApartmentContacts({ data: { apartmentId } }),
      listPhoneNumbersForApartment({ data: { apartmentId } }),
      listContacts(),
    ])
...
function EditApartmentPage() {
  ...
  async function handleDelete() {
    setDeleting(true)
    const result = await softDeleteApartment({ data: { id: apartmentId } })
    setDeleting(false)
    if (!result.ok) {
      toast.error(result.error)
      return
...
function ContactCard({
  ...
  async function handleSaveLink() {
    ...
    const result = await updateContactLink({
      data: { id: link.id, role, isNotificationRecipient: notify },
    })
    setBusy(false)
    if (!result.ok) {
      toast.error(result.error)
  ...
  async function handleUnlink() {
    setBusy(true)
    const result = await unlinkContact({ data: { id: link.id } })
    setBusy(false)
    if (!result.ok) {
      toast.error(result.error)
      return
  ...
  async function handleAddNumber() {
    ...
    const result = await addPhoneNumber({
      data: { contactId: link.contactId, number: newNumber.trim() },
    })
    setBusy(false)
    if (!result.ok) {
      toast.error(result.error)
  ...
  async function handleDeleteNumber(id: number) {
    setBusy(true)
    const result = await deletePhoneNumber({ data: { id } })
    setBusy(false)
    if (!result.ok) {
      toast.error(result.error)
      return
